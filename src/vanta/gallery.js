...
var loadEffect = function (_effectName, loadOptions) {
  ...
  if (effectName === "WAVES") {
    ...
    gui
      .add(options, "birdSize", 0.5, 4)
      .step(0.1)
      .onFinishChange(effect.restart);
    gui.add(options, "wingSpan", 10, 40).step(1).onFinishChange(effect.restart);
    gui
      .add(options, "speedLimit", 1, 10)
      .step(1)
      .onFinishChange(effect.valuesChanger);
    gui
      .add(options, "separation", 1, 100)
      .step(1)
      .onFinishChange(effect.valuesChanger);
    gui
      .add(options, "alignment", 1, 100)
      .step(1)
      .onFinishChange(effect.valuesChanger);
    gui
      .add(options, "cohesion", 1, 100)
      .step(1)
      .onFinishChange(effect.valuesChanger);
    ...
    gui
      .addColor(options, "highlightColor")
      .onFinishChange(effect.updateUniforms);
    gui.addColor(options, "midtoneColor").onFinishChange(effect.updateUniforms);
    gui
      .addColor(options, "lowlightColor")
      .onFinishChange(effect.updateUniforms);
    gui.addColor(options, "baseColor").onFinishChange(effect.updateUniforms);
    gui
      .add(options, "blurFactor", 0.1, 0.9)
      .step(0.01)
      .onFinishChange(effect.updateUniforms);
    gui
      .add(options, "zoom", 0.1, 3.0)
      .step(0.1)
      .onFinishChange(effect.updateUniforms);
    ...
    gui
      ...
      .onFinishChange(effect.updateUniforms);
    gui
      .add(options, "amplitudeFactor", 0.1, 3.0)
      .step(0.1)
      .onFinishChange(effect.updateUniforms);
    gui
      .add(options, "ringFactor", 0.1, 20.0)
      .step(0.1)
      .onFinishChange(effect.updateUniforms);
    gui
      .add(options, "rotationFactor", 0.0, 2.0)
      .step(0.1)
      .onFinishChange(effect.updateUniforms);
    gui.add(options, "speed", 0.0, 5.0).step(0.1);
    ...
    gui
      .addColor(options, "cloudShadowColor")
      .onFinishChange(effect.updateUniforms);
    gui.addColor(options, "sunColor").onFinishChange(effect.updateUniforms);
    gui
      .addColor(options, "sunGlareColor")
      .onFinishChange(effect.updateUniforms);
    gui
      .addColor(options, "sunlightColor")
      .onFinishChange(effect.updateUniforms);
    gui
      .add(options, "speed", 0, 3)
      .step(0.1)
      .onFinishChange(effect.updateUniforms);
    ...
    // gui.addColor(options, 'color2').onFinishChange(effect.updateUniforms)
    gui
      .add(options, "size", 0.1, 3)
      .step(0.1)
      .onFinishChange(effect.updateUniforms);
    gui
      .add(options, "amplitudeFactor", 0, 3)
      .step(0.1)
      .onFinishChange(effect.updateUniforms);
    gui
      .add(options, "xOffset", -0.5, 0.5)
      .step(0.01)
      .onFinishChange(effect.updateUniforms);
    gui
      .add(options, "yOffset", -0.5, 0.5)
      .step(0.01)
      .onFinishChange(effect.updateUniforms);
...
_strk.push(function() {
  ...
  codeStrk = codeStrk
    .replace("[[CODE]]", code)
    .replace("#your-element-selector", ".s-page-1 .s-section-1 .s-section");
  ...
  if (effect.mode == "p5")
    includeCode = $(".usage-for-all .include-p5")[0].innerHTML;
  clone.html(clone.html().replace("[[INCLUDE]]", includeCode));
  clone.html(
    clone.html().replace(/\[\[EFFECTNAME\]\]/g, effectName.toLowerCase()),
  );
  clone.show();
  ...
  if (effect.mode == "p5")
    includeCode = $(".usage-for-strk .include-p5")[0].innerHTML;
  clone.html(clone.html().replace("[[INCLUDE]]", includeCode));
  clone.html(
    clone.html().replace(/\[\[EFFECTNAME\]\]/g, effectName.toLowerCase()),
  );
