...
void main() {
  ...
  // mixedColor *= .875;
  float fadeAmt = 0.0001; // fade this amount each frame // 0.002
  mixedColor = (mixedColor - fadeAmt) * 0.9;
  ...
  float dist = length(pixel - cos(iTime)*0.15) * 8. + sin(iTime) * .01;
  ...
  // float factor = smoothstep(2., .9, edge) * pow(edge, 2.);
  float factor = smoothstep(0., .99, edge) * pow(edge, 8.);
  ...
  float isBackground = step(smoothstep(1., .9, edge), 1.);
